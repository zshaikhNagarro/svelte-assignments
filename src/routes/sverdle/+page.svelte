<script lang="ts">
	...
	function keydown(event: KeyboardEvent) {
		...
		const el = document.querySelector(`[data-key="${event.key}" i]`);
		if (el) el.dispatchEvent(new MouseEvent('click', { cancelable: true, bubbles: true }));
	}
</script>
		...
		{#each Array.from(Array(6).keys()) as row (row)}
			...
			<div class="row" class:current>
				{#each Array.from(Array(5).keys()) as column (column)}
					...
					{const answer = $derived((data.answers[row] && data.answers[row][column]) || '')}
					{const value = $derived((guess && guess[column]) || '')}
